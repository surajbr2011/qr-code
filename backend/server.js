...
// Force restart triggered by .env update
...
app.use(
  cors({
    origin: function (origin, callback) {
      callback(null, true); // Bypass strict CORS to allow external logins
    },
    credentials: true,
  }),
);
...
const io = new Server(server, {
  cors: {
    origin: function (origin, callback) {
      callback(null, true); // Bypass strict CORS for sockets
    },
    methods: ["GET", "POST"],
    credentials: true,
  },
...
server
  ...
  .on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.error(
        `[CRITICAL] Port ${PORT} is already in use. Please kill the process using this port and try again.`,
      );
    } else {
